...
import { style } from 'features/create-article';
...
const ChipsArray: React.FC<IChipsArray> = (props, context) => {
  ...
  const handleKeyPress = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key == 'Enter') {
      ...
      const data = [
        ...chipData,
        { _id: uuidv4().replaceAll('-', '').slice(0, 24), value: currentTag },
      ];
      setChipData(data);
      setCurrentTag('');
    }
  ...
  return (
    <>
      <Paper
        className={style.default.chipArray}
        sx={{
          display: 'flex',
      ...
      <TextField
        classes={{ root: style.default.tags }}
        variant='standard'
