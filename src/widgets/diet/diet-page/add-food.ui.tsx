  ...
  const foods: ProductItem[] = useMemo(
    () =>
      data?.data?.products?.filter(
        (row) => !!row.nutriments && !!Object.keys(row.nutriments).length,
      ) || [],
    [data],
  );
  ...
  return (
    ...
    >
      <Container pos='relative'>
        ...
        <Input.Wrapper style={{ height: '80px' }} label='Введите название'>
          <Input value={search} onChange={(e) => setSearch(e.target.value)} />
        </Input.Wrapper>

        <ActionIcon variant='transparent' onClick={handleSearch} color='red'>
      ...
      <Box style={{ alignSelf: 'end', position: 'fixed', bottom: '10px' }}>
